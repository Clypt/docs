...
```mermaid
flowchart TB
    A["Signal"] --> B["ZScore (normalize)"]
    B --> C["EqualWeight (optimize)"]
    C --> D["BookSize"]
    D --> E["TargetPositionIntention"]
    C --> E
    E --> F["TradingIntention"]
    F --> G["Executor"]
    G --> H["Exchange"]
```
...
```mermaid
flowchart TB
    A["Spread Signal"] --> B["VenueAwareSizing"]
    B --> C["target_notional (all venues)"]
    A --> D["FuturesTargetPositionIntention<br>(futures leg)<br>STATE:binance:futures:*"]
    A --> E["TargetPositionIntention<br>(spot leg)<br>STATE:binance:spot:*"]
```
...
```mermaid
flowchart TB
    A["Spread Signal"] --> B["ArbitrageIntention"]
    B --> C["Long leg (spot)"]
    B --> D["Short leg (futures)"]
    C --> E["Paired TradingIntention"]
    D --> F["Paired TradingIntention"]
    E --> G["Executor<br>(routes to correct venue)"]
    F --> G
```
