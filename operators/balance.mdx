...
Balance operators query the **portfolio state** managed by the execution engine. They read from [STATE](/engine/field-state) inputs (cash, positions, margin) and combine them with [FIELD](/engine/field-state) inputs (current prices) to compute real-time portfolio metrics.
...
- **[STATE Principle](/engine/field-state)**: How STATE data flows through the system
- **[FIELD Data Principle](/engine/field-state)**: Market data used by EquityCalculator
