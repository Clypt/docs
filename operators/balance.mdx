...
```
FIELD (market data)              STATE (portfolio state)
├── close prices ──────┐    ┌── cash balance
├── open prices        │    ├── position quantities
├── high prices        │    ├── entry prices
├── low prices         │    ├── available margin
└── volume             │    └── unrealized PnL
                       │    │
                       ▼    ▼
                 EquityCalculator
                       │
