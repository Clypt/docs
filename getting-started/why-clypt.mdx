...
# Only the mode changes (top-level on TradingSpec)
TradingSpec(..., mode="backtest")   # Historical data + simulated fills
TradingSpec(..., mode="paper")      # Live data + simulated fills (platform-managed)
TradingSpec(..., mode="live")       # Live data + real orders (platform-managed)
```
...
| **Liquidation logic** | Per-exchange margin ratio formulas (Binance, Bybit, Kraken, etc.) |
...
flowchart TB
    ...
    A["Builder submits:<br>Tested on Binance futures"] --> V["Platform verifies"]
    V --> B["Binance futures → Sharpe 1.8<br>(builder's data)"]
    V --> C["Bybit futures → Sharpe 1.5<br>(builder never saw)"]
    V --> D["Gateio futures → Sharpe 1.3<br>(builder never saw)"]
    V --> E["Coinbase futures → Sharpe 0.4<br>(poor — flagged)"]

    class B,C,D success
    class E warn
```

**What the trader sees:** Independently computed metrics, cross-exchange consistency scores, and real-cost impact — not self-reported numbers.
...
4. **Operator library** forms a growing set of composable components
