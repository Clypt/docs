...
flowchart LR
    subgraph Builder
        A["1. Submit strategy<br>(TradingSpec, no source)"]
    end
    subgraph Platform
        B["2. Run backtest on<br>builder's exchange data"]
        C["3. Run backtest on<br>CROSS-EXCHANGE data<br>(builder never saw this)"]
        D["4. Compute metrics<br>(Sharpe, MDD, fees, etc.)"]
        E["5. Publish with<br>Verified badge"]
    end
    subgraph Buyer
        F["6. Buyer deploys<br>(paper → live)"]
    end
    A --> B --> C --> D --> E --> F
```
...
flowchart LR
    ...
    subgraph PV["Platform validates"]
        P1["Reproduce on same data"]
        P2["Run on unseen exchanges"]
        P3["Scale test at 2x/5x/10x"]
        P4["Apply real venue fees"]
    end
    subgraph BY["Buyer sees"]
        Y1["Verified Sharpe 1.98"]
        Y2["Consistency score 0.85"]
        Y3["Capacity: $500K max"]
        Y4["Fee impact: -12% of return"]
    end
    B1 --> P1 --> Y1
    B2 --> P2 --> Y2
    B3 --> P3 --> Y3
