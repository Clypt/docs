...
```mermaid
flowchart TD
    S1["1. Explore<br>Helper.exchanges(), Helper.symbols(), Helper.data_catalog()"]
    S2["2. Build Graph<br>StatefulGraph + operators + FIELD/STATE inputs"]
    S3["3. Backtest<br>TradingExecutionSpec(mode=backtest)"]
    S4["4. Paper Trade<br>TradingExecutionSpec(mode=paper) — 2-4 weeks"]
    S5["5. Live (Small)<br>TradingExecutionSpec(mode=live) — start with 5-10% capital"]
    S6["6. Marketplace<br>Submit TradingSpec for verification and listing"]
...
|-------|--------|--------------|
| **Sharpe ratio** | > 1.0 after costs | `driver.export_results()` |
| **Max drawdown** | < 20% (or your tolerance) | `df_equity.min()` relative analysis |
