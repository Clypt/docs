...
```
1. Explore          → Helper.exchanges(), Helper.symbols(), Helper.data_catalog()
   │
2. Build Graph      → StatefulGraph + operators + FIELD/STATE inputs
   │
3. Backtest         → TradingExecutionSpec(mode="backtest")
   │
4. Paper Trade      → TradingExecutionSpec(mode="paper") — 2-4 weeks
   │
5. Live (Small)     → TradingExecutionSpec(mode="live") — start with 5-10% capital
   │
6. Marketplace      → Submit TradingSpec for verification and listing
```
