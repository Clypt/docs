...
FIELD:futures:close ───────────┤
                    ...
                    ┌──────────┴──────────┐
                    ▼                     ▼
              BasisAlpha            (basis signal)
                    │                     │
          ┌─────── ┼ ─────────────────────┤
          │        │                      │
          ▼        ▼                      ▼
   PairIdGenerator VenueAwareSizing  (per venue)
                        │
            ┌───────────┴───────────┐
            ▼                       ▼
STATE:spot:cash               STATE:futures:cash
STATE:spot:pos                STATE:futures:pos
     │                              │
     ▼                              ▼
EquityCalculator(spot)    EquityCalculator(futures)
     │                              │
     └─────────┬────────────────────┘
               ▼
     ...
     ┌─────────┴─────────┐
     ▼                    ▼
TargetPosition     FuturesTargetPosition
 Intention(spot)    Intention(futures)
     │                    │
     ▼                    ▼
  Executor(spot)    Executor(futures)
```
