...
You've spent weeks researching a strategy. The backtest looks promising. You deploy to live trading. Weeks later, live performance diverges significantly from the backtest.
...
The deepest reason ClyptQ achieves code parity is architectural, not accidental. The engine was deliberately modeled on the [Von Neumann architecture](/getting-started/architecture-overview#the-architecture-behind-the-architecture) — the same principle that makes every computer work.

Consider how a CPU operates: the ALU (Arithmetic Logic Unit) executes the same instruction set regardless of whether the data it processes came from an SSD, RAM, a network socket, or a keyboard. The ALU does not know the data source. It does not need to. It simply processes the bits it receives according to its instruction.

ClyptQ operators work the same way. An SMA operator computes a simple moving average on whatever TaggedArray it receives. It does not know — and cannot know — whether that TaggedArray was filled from a Parquet file (backtest), a WebSocket feed (live), or a simulated stream (paper). The `compute()` interface is the same. The data format is the same. The result is the same.
...
spec = TradingSpec(
    ...
    mode="backtest",                  # ← Only this changes (top-level)
)
```
...
```python
# Backtest → Paper → Live
spec.mode = "paper"   # Paper trading with real data (platform-managed)
spec.mode = "live"    # Live trading with real money (platform-managed)
```
...
flowchart TB
    ...
    A["Fetch next tick<br>Backtest: Parquet<br>Live: WebSocket"] --> B["Distribute FIELD data<br>to consumer buffers"]
    B --> C["Distribute STATE data<br>to consumer buffers"]
    C --> D["Execute operators<br>in topological order"]
    D --> E["Intention nodes<br>compute deltas"]
    E --> F["Executor: process fill<br>Backtest: simulate<br>Live: real order"]
    F --> G["Update STATE<br>with fill result"]
    G --> H["Yield result"]
    class A source
    class H action
```
...
- **[FIELD Protocol](/engine/field-state)**: Data routing that's source-agnostic
- **[STATE Protocol](/engine/field-state)**: Portfolio state that flows identically regardless of execution mode
