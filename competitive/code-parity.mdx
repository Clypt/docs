...
```
Research Path:
  Jupyter → pandas/numpy → DataFrame → backtest_engine.py

Production Path:
  live_adapter.py → event_handler.py → order_router.py → exchange_api.py
```
...
for result in driver:
    ┌──────────────────────────────────────────────────┐
    │  1. Fetch next tick                               │
    │     Backtest: read from Parquet                   │
    │     Live: receive from exchange WebSocket          │
    │                                                    │
    │  2. FIELD data → distribute to consumer buffers   │
    │  3. STATE data → distribute to consumer buffers   │
    │  4. Execute operators in topological order         │
    │  5. Intention nodes → compute deltas              │
    │  6. Executor → process fill                       │
    │     Backtest: simulate fill with cost model        │
    │     Live: send order to exchange, await fill       │
    │  7. Update STATE with fill result                 │
    │  8. Yield result to user                          │
    └──────────────────────────────────────────────────┘
```
...
│  BACKTEST                                                        │
│                                                                  │
│  Historical Parquet ─────────────────────────────→ Graph         │
│  (all ticks pre-loaded, warmup auto-computed)                    │
│                                                                  │
│  Timeline: [───warmup───|────────backtest────────]               │
│            no execution   normal execution                       │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│  PAPER                                                           │
│                                                                  │
│  Historical Parquet ──→ Gap Fill ──→ Live WebSocket ──→ Graph   │
│  (warmup phase)         (if history    (real-time ticks)         │
│                          insufficient)                           │
│                                                                  │
│  Timeline: [──history warmup──|─gap─|────live ticks────]        │
│            no execution              simulated fills              │
└─────────────────────────────────────────────────────────────────┘
...
│                                                                  │
│  Timeline: [──history warmup──|─gap─|────live ticks────]        │
│            no execution              real exchange orders         │
└─────────────────────────────────────────────────────────────────┘
```
