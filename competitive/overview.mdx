...
for result in TradingDriver(spec):
...
- **5 exchanges**: Binance, Gate.io, Bybit, Kraken, Coinbase
...
- **Buyers**: Independently verified performance with cross-exchange validation.
- **Investors**: Transparent risk metrics and portfolio-level reporting.
...
|---|---|---|---|---|---|
| **Research = Live** | Yes | No | Partial | Partial | No |
| **Lookahead prevention** | Structural | Manual | Structural | Manual | Manual |
| **Language** | Python (any library) | C#/Python (limited) | Rust/Cython | Python (pandas) | Python (rigid) |
| **Data included** | Yes (5 exchanges) | Partial (delayed) | No | IB data | No |
| **Futures support** | Full | Yes | Yes | Basic | Limited |
| **Marketplace** | Verified | Alpha Streams | No | No | No |
| **Cost modeling** | Exchange-specific | Basic fees | Configurable | IB fees | Basic fees |
| **AI operators** | LLM, WebSearch, Sentiment | No | No | No | No |

## vs Research-Oriented Frameworks

Some open-source projects focus on **specific domains** — RL (FinRL), factor research (Qlib), or quick prototyping (bt, PyAlgoTrade). They solve one problem well but cannot serve as production trading foundations:

| Limitation | FinRL | Qlib | PyAlgoTrade | bt |
|---|---|---|---|---|
| **Backtesting only** | Mostly | Yes | Yes | Yes |
| **Domain-specific** | RL only | ML/factor | Basic event-driven | Vectorized |
| **Simplified cost model** | Yes | Yes | Yes | Yes |
| **No multi-exchange** | Yes | Yes | Yes | Yes |
| **No production execution** | Mostly | Yes | Limited | Yes |

<Info>
ClyptQ's operator architecture is compatible with these tools. Train a model in Qlib or FinRL, export it, and deploy as a ClyptQ operator via `BaseOperator` inheritance. See [ML/DL Integration](/engine/operator-protocol#ml-and-dl-integration).
</Info>
...
1. **Code parity** is built into the architecture ([StatefulGraph](/engine/stateful-graph) + [TradingSpec](/engine/trading-spec)), not bolted on
2. **Lookahead prevention** is structural ([RollingBuffer](/engine/lookback-buffers)), not runtime checks
3. **Cross-exchange validation** requires exchange-agnostic operators ([TaggedArray](/engine/tagged-array)) — an architectural decision from day one
4. **380+ operators** form a growing library that increases switching costs

These cannot be added to existing platforms — they require rebuilding from the ground up.
