...
```
Tick N:   FIELD data → Operators → Signal → Intention → Executor → Fill → STATE update
Tick N+1: Updated STATE + new FIELD → Operators → Signal → ...
```
...
│                  TradingSpec                       │
│  ┌────────────┐ ┌────────────┐ ┌────────────────┐│
│  │ DataSpec   │ │StrategySpec│ │ ExecutionSpec   ││
│  │            │ │            │ │                 ││
│  │ Exchanges  │ │ Graph      │ │ mode: backtest  ││
│  │ Symbols    │ │ Operators  │ │   → paper       ││
│  │ Timeframes │ │ Connections│ │   → live        ││
│  └────────────┘ └────────────┘ └────────────────┘│
└──────────────────────────────────────────────────┘
         │                │                │
         ▼                ▼                ▼
    ┌─────────┐    ┌────────────┐   ┌───────────┐
    │  FIELD   │    │ StatefulGraph│  │ Executor  │
    │ Protocol │───▶│ (DAG)       │──▶│(Memoryless)│
    └─────────┘    └────────────┘   └───────────┘
                         ▲                │
                         │    STATE       │
                         └────────────────┘
