...
Operators see only past data through `RollingBuffer` — a pre-allocated circular buffer that physically cannot contain future data. There is no array of future prices to accidentally index into.
...
| OKX | 0.5% | 0.5% | MMR/Equity × 100 (inverted) |
...
## Comparing Backtest Accuracy

| Feature | ClyptQ | Zipline | Backtrader | Freqtrade | VectorBT |
|---|---|---|---|---|---|
| **Lookahead prevention** | Structural (RollingBuffer) | Manual | Manual | Manual | None (vectorized) |
| **Fee model** | Exchange-specific (CCXT) | Flat rate | Configurable | Flat rate | Flat rate |
| **Funding rates** | Auto-injected, 8h cycle | No | No | No | No |
| **Liquidation** | Exchange-specific MMR | No | No | No | No |
| **Order validation** | Exchange limits (CCXT) | No | No | Basic | No |
| **Margin tracking** | Full (cross/isolated) | No | No | No | No |
| **Orderbook simulation** | LATENT mode | No | No | No | No |
| **TP/SL simulation** | OHLC-aware | No | Basic | Basic | No |

## What This Means in Practice

A typical perpetual futures strategy with 3× leverage on Binance:

| Cost Component | Annual Impact | Without Simulation |
|---|---|---|
| **Taker fees** (0.04%) | ~2-5% of equity | Often underestimated |
| **Funding rates** | ~5-15% of equity | Completely ignored |
| **Liquidation events** | Position wipeout | Never happens |
| **Order rejections** | Missed trades | All trades execute |

The difference between a "50% annual return" backtest (without proper cost modeling) and reality (with all costs simulated) can easily be **30+ percentage points**. ClyptQ's backtesting doesn't just produce prettier charts — it produces results you can actually trust.
