...
This matches the settlement schedule of major exchanges (Binance, Bybit, Gateio, OKX). Some exchanges use different intervals — see [Exchange Specifics](/backtesting/exchange-specifics) for details.
...
| **Kraken** | Full history | Yes | Some pairs use 4h intervals |
| **Coinbase** | Limited (~42 days) | No | Insufficient for long backtests |
...
<Warning>
If you backtest a futures strategy on Coinbase, funding costs will **not** be simulated. Be aware that live P&L will differ from backtest due to unmodeled funding payments.
</Warning>
