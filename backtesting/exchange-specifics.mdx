...
| **Coinbase** | 1 hour | Every hour | More frequent, smaller payments |
| **Kraken** | 4 hours | 00:00, 04:00, 08:00, 12:00, 16:00, 20:00 | 6× per day |
...
- **Limited funding history**: ~42 days (insufficient for long backtests)
- **Live funding API**: Not supported — falls back to rate=0 for paper/live trading

<Warning>
Coinbase funding rate data is limited to approximately 42 days of history. Long-term backtests on Coinbase perpetual will **not** include funding rate simulation. Additionally, the live funding rate API is unsupported — paper and live trading use a fallback rate of 0. Consider using Binance or Bybit for accurate futures backtests.
</Warning>
...
- **4-hour funding**: 6 settlements per day (vs standard 3) — currently only 8h settlements applied in backtest

<Note>
Kraken's 4-hour funding schedule means 6 settlements per day (00:00, 04:00, 08:00, 12:00, 16:00, 20:00 UTC). The current backtest executor only applies funding at the standard 8h boundaries (00:00, 08:00, 16:00), missing three of the six daily settlements. This understates funding costs by approximately half. Per-exchange settlement schedules are planned.

**CCXT note**: `fetch_funding_rate_history()` returns data at 1h intervals for Kraken, but the intermediate values (01h, 02h, 03h, etc.) are **predicted rates**, not actual settlements. Only the 4h boundary values represent real funding charges.
</Note>
