...
1. User override (CostModelSpec in AccountSpec)
     ↓ if not set
2. CCXT auto-fetch (for crypto exchanges)
     ↓ if fetch fails
3. Known venue defaults (for stock brokers)
     ↓ if unknown venue
4. Fallback default (0.1% maker/taker)
```
...
### 3. Known Venue Defaults (Stock Brokers)

For stock brokers that aren't CCXT-compatible:

| Venue | Maker | Taker | Slippage | Notes |
|-------|-------|-------|----------|-------|
| **Alpaca** | 0.0% | 0.0% | 1.0 bps | Commission-free US equities |
| **Interactive Brokers** | 0.0% | 0.005% | 1.0 bps | Ultra-low professional rates |
