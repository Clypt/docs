...
```
exists?  ─── No ──→  Not in domain (delisted/unlisted). Stop here.
  │
  Yes
  │
valid?   ─── No ──→  In domain but no usable value (missing data/warmup). Stop here.
  │
  Yes
  │
updated? ─── No ──→  Valid value, but forward-filled from previous tick.
  │
  Yes
  │
  ▼
  Fresh data. Use with confidence.
```
