...
| `ArbitrageIntention` | Multi-venue | Allocates across venues for arbitrage |
| `DynamicUniverseIntention` | Dynamic | Target positions with runtime symbol selection |
...
| Type | Description | Fee | Slippage |
|------|-----------|-----|----------|
| `MARKET` | Market order | Taker fee | Applied |
| `LIMIT` | Limit order | Maker fee | None (INSTANT mode) |
| `STOP` | Stop market order | Taker fee | Applied |
| `STOP_LIMIT` | Stop limit order | Maker fee | None |
| `TRAILING_STOP` | Trailing stop order | Taker fee | Applied |
...
|---------|----------|-------|------|
| **Fees** | CostModel (CCXT or override) | CostModel (same) | Exchange actual fee |
| **Slippage** | Configured value | Configured value | Real execution price |
| **Funding** | Settlement hours based | Real-time poll + settlement | Exchange auto-applied |
| **Margin validation** | `_validate_margin()` | `_validate_margin()` | Exchange-side validation |
| **Liquidation** | `check_liquidation()` simulation | `check_liquidation()` simulation | Exchange forced liquidation |
| **Balance sync** | N/A | `sync_account_balances()` | `sync_account_balances()` |
| **Orderbook** | Historical data (if available) | Real-time orderbook | Real-time orderbook |
