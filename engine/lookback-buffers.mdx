...
```mermaid
flowchart TB
    A["1. add_node()<br>RollingBuffer allocated<br>(lookback × n_symbols)"]
    B["2. on_tick() — warmup phase<br>buffer.append(tick)<br>valid=False until full"]
    C["3. on_tick() — normal phase<br>overwrites oldest entry<br>operator receives full lookback"]
    D["4. expand_axis()<br>if new symbols appear<br>buffer.pad(pad_count)"]
...
- **[FIELD Data Principle](/engine/field-data-principle)**: FIELD data is distributed to consumer buffers
- **[STATE Principle](/engine/state-principle)**: STATE data follows the same buffer mechanism
