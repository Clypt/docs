...
```
┌──────────────────┐    ┌──────────────────┐    ┌──────────────────┐
│  Operator Graph   │ →  │  Intention Node   │ →  │   Executor        │
│  (Signal → Weights│    │  (Weights → Delta │    │  (Delta → Order   │
│   → Position)     │    │   → Intention)    │    │   → Fill → STATE) │
│                   │    │                   │    │                   │
│  Stateless        │    │  Stateless        │    │  Memoryless       │
│  (pure functions) │    │  (delta compute)  │    │  (no history)     │
└──────────────────┘    └──────────────────┘    └──────────────────┘
         ▲                                                │
         │              STATE feedback                     │
         └─────────────────────────────────────────────────┘
...
The executor has **no memory** of past orders or fills. Every tick:
1. Receives `TradingIntention` from the intention node
2. Computes delta from current STATE (positions, cash)
3. Generates and executes order
4. Updates STATE with fill result
5. **Forgets everything** — next tick starts fresh

This means **all metrics and tracking are done by operators in the graph**, not by the executor. Want a custom win rate calculation? Write a metric operator. Want drawdown tracking? Write a metric operator. The execution layer is a pure state machine with no opinion about performance measurement.
...
- **[FIELD Data Principle](/engine/field-data-principle)**: Market data inputs
- **[STATE Principle](/engine/state-principle)**: Portfolio state inputs
- **[Operator Reference](/operators/overview)**: Complete catalog of 213+ built-in operators
