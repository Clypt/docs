...
ClyptQ's engine is built on the same principle that underpins every modern computer: the [Von Neumann separation](/getting-started/architecture-overview#the-architecture-behind-the-architecture) of stateless computation from stateful memory.
...
flowchart TB
    ...
    OnTick --> Step1["Distribute source inputs<br>For each FIELD/STATE source:<br>buffer.append(tick)"]
    Step1 --> Step2["Execute operators<br>in topological order<br>distribute outputs to buffers"]
    Step2 --> Step3["Return all outputs"]

    class OnTick highlight
    class Step1 source
    class Step3 action
```
...
This is the [Von Neumann principle](/getting-started/architecture-overview#the-architecture-behind-the-architecture) in action: computation units (operators) are pure functions with no memory, while the storage system (the graph) manages all state. This architectural split — not any individual feature — is what makes code parity across modes possible.
...
- **[FIELD & STATE](/engine/field-state)**: FIELD and STATE sources are distributed to consumer buffers by the graph
