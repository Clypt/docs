...
```python
OHLCVSpec(exchange=["binance", "bybit"], market_type=["futures", "linear"], timeframe="1m")
```
...
<Note>
Coinbase does **not** support `4h`. It supports `2h` and `6h` instead. Check [Supported Exchanges](/platform/supported-exchanges) for the full per-exchange timeframe matrix.
</Note>
