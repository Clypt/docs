...
```
TradingDataSpec                          StatefulGraph
┌───────────────────────┐    ┌─────────────────────────────────┐
│ symbol_source_map     │    │                                 │
│ observations          │───▶│  FIELD:binance:futures:ohlcv:   │
│   └─ OHLCVSpec        │    │    close, open, high, low, vol  │
│ start / end           │    │                                 │
└───────────────────────┘    │  → RollingBuffer → Operator     │
                             └─────────────────────────────────┘
```
...
```
TradingDataSpec(start, end)
        │
        ▼
  Load Parquet files for [start, end] range
        │
        ▼
  Validate: coverage, NaN ratio, time range
        │
        ▼
  Forward-fill missing bars
        │
        ▼
  Feed into RollingBuffers → Graph
```
...
```
TradingDataSpec (no start/end)
        │
        ▼
  1. Historical warmup
     Load last N bars from Parquet
     (N = max lookback across graph)
        │
        ▼
  2. Gap fill (if needed)
     If historical data doesn't reach current time,
     fetch missing bars from exchange REST API
        │
        ▼
  3. Clock sync
     Align to next bar boundary
        │
        ▼
  4. Live stream
     WebSocket feed from exchange
     Each new bar → tick through graph
```
