...
```
Registration → Qualifying Round → Semi-Finals → Finals → Capital Allocation
     │              │                  │            │           │
  Submit         Backtest           Paper         Live     Top performers
  strategy      validation        trading       trading    get AUM
```
...
2. **Scale testing** — Tested at 2x/5x/10x the declared capital
3. **Cost model verification** — Realistic fees, slippage, funding rates applied
4. **Lookahead bias check** — Structural verification via ClyptQ's RollingBuffer architecture
...
| **Scale testing** | Strategies that only work at tiny scale |
